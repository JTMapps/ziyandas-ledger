...
//
// ─── THE LOOP FIX ────────────────────────────────────────────────────────────
// The previous version had `ensureCoA` in the useEffect dep array.
// Even though useEnsureCoA now returns a stable ensureCoA, we pin it via a
// ref so the effect deps are purely [entityId] — a string primitive. This
// guarantees no re-run loop regardless of any future change to useEnsureCoA.
//
// Flow:
//   entityId changes → effect fires once → ensureCoARef.current() runs →
//   finally: setReady(true) → wizard renders.
...
function AmountField({ value, onChange, disabled }: { value: string; onChange: (v: string) => void; disabled: boolean }) {
  return (
    <div className="space-y-1">
...
function JournalPreview({ lines, amount }: { lines: { side: "Dr" | "Cr"; label: string }[]; amount: number }) {
    ...
    <div className="rounded-lg bg-gray-50 border text-xs font-mono p-3 space-y-1 text-gray-600">
      ...
      {lines.map((line, i) => (
        <div key={i} className="flex justify-between">
          <span className={line.side === "Cr" ? "pl-8" : ""}>{line.side} {line.label}</span>
          <span>{formatted}</span>
        </div>
      ))}
    </div>
...
function WizardShell({ title, subtitle, ifrsRef, children, canSubmit, isPending, errorMsg, onSubmit, onCancel }: {
  title: string; subtitle: string; ifrsRef: string; children: React.ReactNode;
  canSubmit: boolean; isPending: boolean; errorMsg: string | null;
  onSubmit: () => void; onCancel: () => void;
}) {
  return (
    <div className="max-w-lg mx-auto space-y-5 pb-8">
      <div>
        <button onClick={onCancel} className="text-sm text-gray-400 hover:text-gray-700 mb-3 flex items-center gap-1">
          ← Ledger
        </button>
      ...
      {children}
      {errorMsg && (
        <div className="text-sm text-red-700 rounded bg-red-50 border border-red-200 p-3">{errorMsg}</div>
      )}
...
function CashSaleForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  return (
    <WizardShell title="Cash Sale" subtitle="Revenue received in cash" ifrsRef="IFRS 15"
      canSubmit={canSubmit} isPending={record.isPending} errorMsg={errToString(record.error)}
      onCancel={onCancel}
      onSubmit={() => record.mutate({
        ...
        effects: [
          ...
          { account_id: revId,  amount: parsed, effect_sign: -1 },
        ],
      }, { onSuccess: onCancel })}>
      ...
      <div>
        ...
        <AccountPicker entityId={entityId} value={cashId} onChange={setCashId} allowedTypes={["ASSET"]} cashOnly leafOnly disabled={record.isPending} />
      </div>
      <div>
        <PickerLabel label="Cr — Revenue account" hint="Income account for this type of sale" />
        <AccountPicker entityId={entityId} value={revId} onChange={setRevId} allowedTypes={["INCOME"]} leafOnly disabled={record.isPending} />
      </div>
    </WizardShell>
  );
}
  ...
  return (
    <WizardShell title="Credit Sale" subtitle="Invoice customer — collect later" ifrsRef="IFRS 15"
      ...
      onSubmit={() => record.mutate({
        ...
        effects: [
          { account_id: recId, amount: parsed, effect_sign: 1 },
          { account_id: revId, amount: parsed, effect_sign: -1 },
        ],
      }, { onSuccess: onCancel })}>
      ...
      <div>
        ...
        <AccountPicker entityId={entityId} value={recId} onChange={setRecId} allowedTypes={["ASSET"]} cashOnly={false} leafOnly disabled={record.isPending} />
      </div>
      <div>
...
function CashExpenseForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  const record = useRecordEconomicEvent();
  const canSubmit = !record.isPending && !!expId && !!cashId && parsed > 0 && !!taxTreatment;
  ...
  return (
    <WizardShell title="Cash Expense" subtitle="Expense paid directly from cash" ifrsRef="IAS 1"
      ...
      onSubmit={() => record.mutate({
        ...
        effects: [
          { account_id: expId,  amount: parsed, effect_sign: 1,  deductible: true,  tax_treatment: taxTreatment },
          { account_id: cashId, amount: parsed, effect_sign: -1, deductible: false, tax_treatment: null },
        ],
      }, { onSuccess: onCancel })}>
      ...
      <TaxTreatmentSelect value={taxTreatment} onChange={setTaxTreatment} required disabled={record.isPending}
        label="Tax treatment (expense)" hint="Required because this line is deductible in the database" />
      <div>
        <PickerLabel label="Dr — Expense account" hint="The type of expense incurred" />
        <AccountPicker entityId={entityId} value={expId} onChange={setExpId} allowedTypes={["EXPENSE"]} leafOnly disabled={record.isPending} />
      </div>
      <div>
...
function ExpenseOnCreditForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  const [taxTreatment, setTaxTreatment] = useState("");
  const record = useRecordEconomicEvent();
  const canSubmit = !record.isPending && !!expId && !!payId && parsed > 0 && !!taxTreatment;
  ...
  return (
    <WizardShell title="Expense on Credit" subtitle="Bill received — pay later" ifrsRef="IAS 37"
      ...
      onSubmit={() => record.mutate({
        ...
        effects: [
          { account_id: expId, amount: parsed, effect_sign: 1,  deductible: true,  tax_treatment: taxTreatment },
          { account_id: payId, amount: parsed, effect_sign: -1, deductible: false, tax_treatment: null },
        ],
      }, { onSuccess: onCancel })}>
      ...
      <div>
        ...
        <AccountPicker entityId={entityId} value={payId} onChange={setPayId} allowedTypes={["LIABILITY"]} leafOnly disabled={record.isPending} />
      </div>
    </WizardShell>
  );
}
  ...
  return (
    <WizardShell title="Asset Purchase" subtitle="Buy a long-term asset for cash" ifrsRef="IAS 16"
      ...
      onSubmit={() => record.mutate({
        ...
        effects: [
          { account_id: assetId, amount: parsed, effect_sign: 1 },
          { account_id: cashId,  amount: parsed, effect_sign: -1 },
        ],
      }, { onSuccess: onCancel })}>
      ...
      <div>
        ...
        <AccountPicker entityId={entityId} value={assetId} onChange={setAssetId} allowedTypes={["ASSET"]} cashOnly={false} leafOnly disabled={record.isPending} />
      </div>
      <div>
...
function DepreciationForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  const [taxTreatment, setTaxTreatment] = useState("");
  const record = useRecordEconomicEvent();
  const canSubmit = !record.isPending && !!deprExpId && !!accumId && parsed > 0 && !!taxTreatment;
  ...
  return (
    <WizardShell title="Depreciation" subtitle="Allocate asset cost to expense this period" ifrsRef="IAS 16"
      ...
      onSubmit={() => record.mutate({
        ...
        effects: [
          { account_id: deprExpId, amount: parsed, effect_sign: 1,  deductible: true,  tax_treatment: taxTreatment },
          { account_id: accumId,   amount: parsed, effect_sign: -1, deductible: false, tax_treatment: null },
        ],
      }, { onSuccess: onCancel })}>
      ...
      <TaxTreatmentSelect value={taxTreatment} onChange={setTaxTreatment} required disabled={record.isPending}
        label="Tax treatment (depreciation expense)" hint="Required because this line is deductible in the database" />
      <div>
        <PickerLabel label="Dr — Depreciation Expense account" hint="An expense account for depreciation charges" />
        <AccountPicker entityId={entityId} value={deprExpId} onChange={setDeprExpId} allowedTypes={["EXPENSE"]} leafOnly disabled={record.isPending} />
      </div>
      <div>
        <PickerLabel label="Cr — Accumulated Depreciation account" hint="Contra-asset (ASSET type) that offsets PPE cost" />
        <AccountPicker entityId={entityId} value={accumId} onChange={setAccumId} allowedTypes={["ASSET"]} cashOnly={false} leafOnly disabled={record.isPending} />
      </div>
    </WizardShell>
  );
}
  ...
  return (
    <WizardShell title="Loan Received" subtitle="Borrow funds — creates a liability" ifrsRef="IFRS 9"
      ...
      <div>
        ...
        <AccountPicker entityId={entityId} value={loanId} onChange={setLoanId} allowedTypes={["LIABILITY"]} leafOnly disabled={record.isPending} />
      </div>
    </WizardShell>
  );
}
...
function LoanRepaidForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  return (
    <WizardShell title="Loan Repaid" subtitle="Repay principal — record interest separately" ifrsRef="IFRS 9"
      canSubmit={canSubmit} isPending={record.isPending} errorMsg={errToString(record.error)}
      onCancel={onCancel}
      onSubmit={() => record.mutate({
...
function OwnerInvestmentForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  return (
    <WizardShell title="Owner Investment" subtitle="Capital contributed — not income" ifrsRef="IAS 32"
      canSubmit={canSubmit} isPending={record.isPending} errorMsg={errToString(record.error)}
      onCancel={onCancel}
      onSubmit={() => record.mutate({
      ...
      <div>
        ...
        <AccountPicker entityId={entityId} value={equityId} onChange={setEquityId} allowedTypes={["EQUITY"]} leafOnly disabled={record.isPending} />
      </div>
    </WizardShell>
  );
}
...
function OwnerWithdrawalForm({ entityId, onCancel }: { entityId: string; onCancel: () => void }) {
  ...
  return (
    <WizardShell title="Owner Withdrawal" subtitle="Drawings — reduces equity, not an expense" ifrsRef="IAS 32"
      ...
      <div>
        <PickerLabel label="Dr — Drawings / Equity account" hint="Drawings or Retained Earnings. Reduces equity." />
        <AccountPicker entityId={entityId} value={drawId} onChange={setDrawId} allowedTypes={["EQUITY"]} leafOnly disabled={record.isPending} />
      </div>
      <div>
...
export default function GeneralCaptureWizard() {
  ...
  // Pin ensureCoA in a ref — effect deps are purely [entityId] (a primitive).
  // This prevents any future change in useEnsureCoA from accidentally
  // reintroducing a dep-change loop.
  const ensureCoARef = useRef(ensureCoA);
  ensureCoARef.current = ensureCoA;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await ensureCoARef.current();
      } finally {
        if (runId.current === id) setReady(true);
      }
    })();
  }, [entityId]); // ← only entityId. No functions. No objects.
    ...
    return (
      <div className="max-w-lg mx-auto p-6 space-y-4">
        ...
        <p className="text-sm text-gray-500">
          <code className="bg-gray-100 px-1 rounded">{eventType || "(none)"}</code>{" "}
          is not a recognised general capture type. Valid types: {Object.keys(WIZARD_MAP).join(", ")}.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-6">
      <WizardForm entityId={entityId} onCancel={() => navigate(`/entities/${entityId}/ledger`)} />
    </div>
  );
}